...
SELECT f.name, concat(m.firstname, ' ', m.surname) as full_name, (f.guestcost * fac_slot.guest_slot + f.membercost * fac_slot.mem_slot) as tot_cost
from (
...
select strftime('%m', starttime) as month_group, facid, sum(slots) as usage
from Bookings
where memid != 0
